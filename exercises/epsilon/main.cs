...
public static class main{
	static void MaxInt(){
		//int i=int.MaxValue/10*9;
		int i=1;
		while(i+1>i) {i++;}
		WriteLine("my max int = {0}\n",i);
	}
	
	static void MinInt(){
		int i=1;
		while(i-1<i) {i--;}
		WriteLine($"my min int = {0}\n",i);
	}
	...
	static void MachineEpsilon(){
		...
		WriteLine($"Machone epsilon for double: {x}");
	...
	static void testapprox(){
		...
		b=2;
		WriteLine($"approx(1, 1+1e-9) = {approx(a,b)} should be False");
	}
